...
- **Automated User ID Generation**: Removed the user_id field from the interface. User IDs are now automatically generated by the system.
- **ID Number-Based Storage**: The system now prioritizes using the extracted ID number from documents as the primary identifier for MongoDB storage.
- **Enhanced Logging**: Added comprehensive logging for MongoDB operations.
...
- **Auto-Generated User IDs**: Automatically assigns unique IDs for document organization
- **Document ID Number Storage**: Uses extracted ID numbers from documents as primary identifiers
...
#### Get Document by ID Number

```bash
curl "http://localhost:8000/api/documents/id/{id_number}"
```
...
# Get document by ID number (extracted from the document)
id_number = "28140001175"  # Replace with ID extracted from document
response = requests.get(f'http://localhost:8000/api/documents/id/{id_number}')
